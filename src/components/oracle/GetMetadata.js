...
const GetMetadata = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
          setLoading(true);
          try {
              const response = await ConexionService.getMetadata(conexion.url , conexion.schema, conexion.usuario, conexion.contraseña );
    ...
    const generate = (e, a) => {
        ...
        pjl.forEach( (a) => {
            ConexionService.generarMetaColumnas("oracle", url , schema, conexion.usuario, conexion.contraseña, a);
            const relacion = ConexionService.validarRelaciones(url , schema , conexion.usuario, conexion.contraseña, a, "oracle");
  ...
  return (
    <div className='container mx-auto my-8'>
        <div className='h-12'>
            <button onClick={() => navigate("/ConexionList")} className='rounded bg-slate-600 text-white px-6 py-2 font-semibold'>Atras</button>
        </div>
