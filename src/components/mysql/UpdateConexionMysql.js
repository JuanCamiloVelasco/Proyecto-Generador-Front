...
const UpdateConexion = () => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
            const response = await ConexionServiceMysql.getConexionById(conexion.id);
    ...
    const updateConexion = (e) => {
        e.preventDefault();
        ConexionServiceMysql.updateConexion(conexion, id).then((response)=> {
            navigate("/conexionListMysql");
        }).catch((error) => {
            console.log(error);
        });
    };
    ...
    <div className='flex max-w-2xl mx-auto shadow'>
        <div className='px-8 py-8 bg-orange-50 background-color: oldlace;'>
            ...
            <div className='items-center justify-center h-14 w-full my-4 space-x-4 pt-4'>
                ...
                <button onClick={() => navigate("/conexionListMysql")} className='rounded text-white font-semibold bg-red-400 hover:bg-red-700 py-2 px-2'>Cancelar</button>
            </div>
        </div>
    </div>
  );
