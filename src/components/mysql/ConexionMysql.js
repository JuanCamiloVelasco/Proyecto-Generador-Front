...
const Conexion = ( {conexion, deleteConexion} ) => {
    ...
    const editConexion = (e, id) => {
        e.preventDefault();
        navigate(`/editConexionMysql/${id}`);
    };

    const metadata = (e, url, es, usuario, contraseña) => {
        e.preventDefault();
        navigate(`/getMetaMysql/${url}/${es}/${usuario}/${contraseña}`);
    };
  ...
  return (
    <tr key={conexion.id}>
        ...
        <td className='text-left px-6 py-4 whitespace-nowrap'>
            <div className='text-sm text-gray-500'>{conexion.es}</div>
        </td>
        <td className='text-right px-6 py-4 whitespace-nowrap font-medium text-sm'>
            ...
            <a onClick={(e, url, es, usuario, contraseña) => metadata(e, conexion.url, conexion.es, conexion.usuario, conexion.contraseña)} className='text-purple-500 hover:text-indigo-800 px-4 hover:cursor-pointer'>MetaData</a>
