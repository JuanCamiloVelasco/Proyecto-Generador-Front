...
import ConexionServicePost from '../../services/ConexionServicePost';
...
const GetMetadata = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
          setLoading(true);
          try {
              const response = await ConexionServicePost.getMetadata(conexion.url , conexion.schema, conexion.usuario, conexion.contraseña );
    ...
    const generate = (e, a) => {
        ...
        pjl.forEach( (a) => {
            ConexionServicePost.generarMetaColumnas("postgres", url , schema, conexion.usuario, conexion.contraseña, a);
            const relacion = ConexionServicePost.validarRelaciones(url , schema , conexion.usuario, conexion.contraseña, a, "postgres");
  ...
  return (
    <div className='container mx-auto my-8'>
        <div className='h-12'>
            <button onClick={() => navigate("/ConexionListPost")} className='rounded bg-slate-600 text-white px-6 py-2 font-semibold'>Atras</button>
        </div>
