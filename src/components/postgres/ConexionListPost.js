...
import Conexion from './ConexionPost';
...
const ConexionList = () => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        setLoading(true);
        try {
            const response = await ConexionServicePost.getConexiones();
    ...
    const deleteConexion = (e, id) => {
        e.preventDefault();
        ConexionServicePost.deleteConexion(id).then((res) => {
            if(conexionespost) {
  ...
  return (
    ...
    <div className="h-16 px-8 flex items-center bg-gray-800" >
      <p className="text-white font-bold">Sistema de Conexiones Postgresql</p>
    </div>
    <div className='container mx-auto my-8'>
        <div className='h-12'>
            <button onClick={() => navigate("/addConexionPost")} className='rounded bg-slate-600 text-white px-6 py-2 font-semibold'>Añadir Conexion</button>
        </div>
