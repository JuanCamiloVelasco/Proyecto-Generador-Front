...
const UpdateConexion = () => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
            const response = await ConexionServicePost.getConexionById(conexion.id);
    ...
    const updateConexion = (e) => {
        e.preventDefault();
        ConexionServicePost.updateConexion(conexion, id).then((response)=> {
            navigate("/conexionListPost");
        }).catch((error) => {
            console.log(error);
        });
    };
        ...
        <div className='px-8 py-8 bg-orange-50 background-color: oldlace;'>
            ...
            <div className='items-center justify-center h-14 w-full my-4'>
                <div className='input-field'>
                    ...
                    <input type='text' name="es" value={conexion.es} onChange={(e) => handleChange(e)} className='block text-gray-600 text-sm font-normal'></input>
                </div>
            </div>
            <div className='items-center justify-center h-14 w-full my-4'>
                <div className='input-field'>
            ...
            <div className='items-center justify-center h-14 w-full my-4 space-x-4 pt-4'>
                ...
                <button onClick={() => navigate("/conexionListPost")} className='rounded text-white font-semibold bg-red-400 hover:bg-red-700 py-2 px-2'>Cancelar</button>
            </div>
        </div>
    </div>
  );
